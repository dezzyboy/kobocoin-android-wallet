...
import com.machadocrypto.kobocoinj.core.Address;
import com.machadocrypto.kobocoinj.core.AddressFormatException;
import com.machadocrypto.kobocoinj.core.ECKey;
import com.machadocrypto.kobocoinj.core.NetworkParameters;
import com.machadocrypto.kobocoinj.core.Sha256Hash;
import com.machadocrypto.kobocoinj.core.Transaction;
import com.machadocrypto.kobocoinj.core.TransactionConfidence;
import com.machadocrypto.kobocoinj.core.TransactionConfidence.ConfidenceType;
import com.machadocrypto.kobocoinj.core.Wallet;
import com.machadocrypto.kobocoinj.core.Wallet.BalanceType;
import com.machadocrypto.kobocoinj.core.Wallet.SendRequest;

import com.machadocrypto.kobocoin_android_wallet.AddressBookProvider;
import com.machadocrypto.kobocoin_android_wallet.Configuration;
import com.machadocrypto.kobocoin_android_wallet.Constants;
import com.machadocrypto.kobocoin_android_wallet.ExchangeRatesProvider;
import com.machadocrypto.kobocoin_android_wallet.ExchangeRatesProvider.ExchangeRate;
import com.machadocrypto.kobocoin_android_wallet.WalletApplication;
import com.machadocrypto.kobocoin_android_wallet.data.PaymentIntent;
import com.machadocrypto.kobocoin_android_wallet.data.PaymentIntent.Standard;
import com.machadocrypto.kobocoin_android_wallet.integration.android.KOBOcoinIntegration;
import com.machadocrypto.kobocoin_android_wallet.offline.DirectPaymentTask;
import com.machadocrypto.kobocoin_android_wallet.ui.InputParser.BinaryInputParser;
import com.machadocrypto.kobocoin_android_wallet.ui.InputParser.StreamInputParser;
import com.machadocrypto.kobocoin_android_wallet.ui.InputParser.StringInputParser;
import com.machadocrypto.kobocoin_android_wallet.util.Bluetooth;
import com.machadocrypto.kobocoin_android_wallet.util.GenericUtils;
import com.machadocrypto.kobocoin_android_wallet.util.Nfc;
import com.machadocrypto.kobocoin_android_wallet.util.PaymentProtocol;
...
public final class SendCoinsFragment extends SherlockFragment
{
	...
	public View onCreateView(final LayoutInflater inflater, final ViewGroup container, final Bundle savedInstanceState)
	{
		...
		final CurrencyAmountView KOBOAmountView = (CurrencyAmountView) view.findViewById(R.id.send_coins_amount_kobo);
		KOBOAmountView.setCurrencySymbol(config.getKOBOPrefix());
		KOBOAmountView.setInputPrecision(config.getKOBOMaxPrecision());
		KOBOAmountView.setHintPrecision(config.getKOBOPrecision());
		KOBOAmountView.setShift(config.getKOBOShift());
		...
		amountCalculatorLink = new CurrencyCalculatorLink(KOBOAmountView, localAmountView);
	...
	private void handleGo()
	{
		...
		new SendCoinsOfflineTask(wallet, backgroundHandler)
		{
			...
			protected void onInsufficientMoney(@Nullable final BigInteger missing)
			{
				...
				final int KOBOShift = config.getKOBOShift();
				final int KOBOPrecision = config.getKOBOMaxPrecision();
				final String KOBOPrefix = config.getKOBOPrefix();
				...
				if (missing != null)
					msg.append(
							getString(R.string.send_coins_fragment_insufficient_money_msg1,
									KOBOPrefix + ' ' + GenericUtils.formatValue(missing, KOBOPrecision, KOBOShift))).append("\n\n");
				if (pending.signum() > 0)
					msg.append(
							getString(R.string.send_coins_fragment_pending,
									KOBOPrefix + ' ' + GenericUtils.formatValue(pending, KOBOPrecision, KOBOShift))).append("\n\n");
	...
	private void handleEmpty()
	{
		...
		amountCalculatorLink.setKOBOAmount(available);
	...
	private void updateView()
	{
		if (paymentIntent != null)
		{
			...
			if (sentTransaction != null)
			{
				final int KOBOPrecision = config.getKOBOPrecision();
				final int KOBOShift = config.getKOBOShift();

				sentTransactionView.setVisibility(View.VISIBLE);
				sentTransactionListAdapter.setPrecision(KOBOPrecision, KOBOShift);
	...
	private void updateStateFrom(final @Nonnull PaymentIntent paymentIntent)
	{
		...
		handler.post(new Runnable()
		{
			...
			public void run()
			{
				if (state == State.INPUT)
				{
					amountCalculatorLink.setKOBOAmount(paymentIntent.getAmount());
